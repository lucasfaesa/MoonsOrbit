		...
		if (Application.platform == RuntimePlatform.Android ||
			...
			if (Mouse.current.leftButton.wasReleasedThisFrame) {
				...
				if (Mouse.current.leftButton.isPressed) {
					Vector2 p = new Vector2(Mouse.current.position.x.value, Mouse.current.position.y.value);
					if (gestureDetector.Count == 0 || (p - gestureDetector[gestureDetector.Count - 1]).magnitude > 10)
						gestureDetector.Add(p);
				}
			}
	...
	Vector2 getDownPos()
	{
		if (Application.platform == RuntimePlatform.Android ||
		   Application.platform == RuntimePlatform.IPhonePlayer) {
			...
			if (Mouse.current.leftButton.wasPressedThisFrame) {
				downPos.x = Mouse.current.position.x.value;
				downPos.y = Mouse.current.position.y.value;
				return downPos;
			}
		}
